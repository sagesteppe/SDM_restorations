...
hu12 <- rast('../../Geospatial_data/WBD_rast/hu12R-line.tif')
plot(hu12)
...
patchTaggR <- function(x){
  ...
  data.frame(
    ...
    write.csv(row.names = F, 
              file.path(fp, paste0(r_taxon, '-summary', '.csv')))
  ...
  dplyr::distinct(occ_pops, PatchID, .keep_all = TRUE) |> # this is multiple pts per patch - not necessary for us. 
    ...
    write.csv(row.names = F,
            file.path(fp, paste0(r_taxon, '-topSuitability', '.csv')))
  ...
  # identify the drainages within the HU12 database
  drain_nos <- terra::extract(r_drain, marginal_pops, bind = TRUE) |>
    sf::st_as_sf()
  r_drain_sub <- terra::mask()
  
  # mask original suitability map to these areas, and filter >60% suitable habitat
  f1 <- list.files('suitability_maps')
  
  suitable <- terra::mask(suitable, r_drain_sub)
  msk <- ifel(suitable > 55 & suitable < 80, NA, 1) # let's make sure these areas do not include existing clusters. 
  suitable <- mask(suitable, msk) # this is now a second raster for occupied marginal populations
  
  writeRaster(paste0('../results/marginal_habitat', r_taxon, '.tif'))
  return(marginal_pops)
}
