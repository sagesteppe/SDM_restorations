...
extractR <- function(x){ 
  ...
  wide <- dplyr::mutate(
    ...
    .before = geometry) |>
    dplyr::select(-f_rankedp, -f_patchMetrics, -f_classMetrics) |>
...
We will discard any point within 500m of an historic point which we know was distributed to the crews via QGIS. 
```{r}
...
strip_occurrences <- function(x){
  ...
  occurrences <- sf::st_read(sf::st_drop_geometry(x$f_rankedp[1]), quiet = T) |>
    sf::st_buffer(1000)
  ...
  return(dplyr::select(x, -f_rankedp))
}
