...
habitat_specialist <- data.frame(
  suitability = c(
    seq(from= 0, to  = 0.1, length.out = 70), 
    seq(from= 0.1, to  = 0.4, length.out = 15), 
    seq(from= 0.4, to  = 0.8, length.out = 10),
    seq(from= 0.8, to  = 1, length.out = 5)
  ),
  percent = 1:100
)

habitat_generalist <- data.frame(
  suitability = c(
    seq(from= 0, to  = 0.5, length.out = 40), 
    seq(from= 0.6, to  = 0.7, length.out = 30), 
    seq(from= 0.7, to  = 0.9, length.out = 20),
    seq(from= 0.9, to  = 1, length.out = 10)
  ),
  percent = 1:100
)
...
ggplot() + 
  geom_smooth(data = habitat_specialist, aes(x = percent, y = suitability)) + 
  geom_smooth(data = habitat_generalist, aes(x = percent, y = suitability)) +
  theme(
...
y1 <- rnorm(1:10, 100, 10)
elucidate::mean_ci(y1)

recs <- filter(suitability_amounts, Percent == 50) |>
  pull(Records)

as.numeric( elucidate::mean_ci(recs, replicates = 1000, ci_level = 0.99) )
...
ggplot(suitability_amounts,
  ...
  geom_ribbon(data = suitability_means, aes(ymin = lower, ymax = upper, x = Percent),
              fill = '#73BA9B', inherit.aes = F, alpha = 0.5) + 
  geom_line(data = suitability_means, 
            aes(x = Percent, y =  mean, color = 'a'), 
            color = '#093824', inherit.aes = F, lwd = 1) + 
  geom_line(data = suitability_means, aes(x = Percent, y = Null_mod/100),
            color = 'black', inherit.aes = F,  lwd = 1) +
  geom_line(data = fp, aes(x = suitability*100, y = obs),
            color = '#FF2C55', inherit.aes = F,  lwd = 1) +
  
  labs(
    title = 'Populations are found in\nareas with higher suitability',
    x = "Raster Cells (cumulative)",
    y = 'Predicted Suitability (cumulative)') + 
  theme_classic() +
  scale_y_continuous(breaks = seq(0, 1, by = 0.2), sec.axis = sec_axis(~.*100,
    name = 'Populations Found (cumulative)',
...
```{r}
suitability_means[ which.min(abs(suitability_means$Percent-50)), 'mean'] # on average 50% of cells have lower probabilities of being suitable than 0.23

suitability_means[ which.min(abs(suitability_means$Percent-80)), 'mean'] # on average 80% of cells have lower probabilities of being suitable than 0.614


fp[ which.min(abs(fp$obs-0.2)), ] # 80% of all populations are found in areas with suitability > 65.2% 

fp[ which.min(abs(fp$obs-0.5)), ] # 50% of all populations are found in areas with suitability > 83.6% 

fp[ which.min(abs(fp$obs-0.8)), ] # the final 20 % of populations are found in areas with suitability > 94.6%
```
