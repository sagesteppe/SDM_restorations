...
ggplot(suitability_amounts, aes(y = Percent, x = Records, color = Species, group = Species)) +
  geom_line(lty = 1) +
  scale_color_grey(guide = 'none') + 
  geom_ribbon(data = suitability_means, aes(xmin = lower, xmax = upper, y = Percent),
              fill = '#73BA9B', inherit.aes = F, alpha = 0.5) + 
  geom_line(data = suitability_means, 
            aes(y = Percent, x =  mean, color = 'a'), color = '#093824', inherit.aes = F, lwd = 1) + 
  geom_line(data = fp, aes(y = suitability*100, x = obs), color = '#FF2C55', inherit.aes = F,  lwd = 1) +
  
  labs(
    title = 'Populations are found in areas\nwith higher predicted suitability',
    y = "Raster Cells (cumulative)",
    x = 'Predicted Suitability (cumulative)') + 
  theme_classic() +
  scale_x_continuous(breaks = seq(0, 1, by = 0.2), sec.axis = sec_axis(~.*100,
    name = 'Populations Found (cumulative)',
    breaks = seq(0, 100, by = 20), 
    labels = \(x) paste0(x, "%"))
  )  +
  
  scale_y_continuous(breaks = seq(0, 100, by = 20), labels = \(x) paste0(x, "%")) + 
  theme(
