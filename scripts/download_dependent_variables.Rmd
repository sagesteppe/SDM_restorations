...
aim <- st_read(file.path(base_p, 'AIM', 'AIM_records.shp')) %>% 
...
records <- st_transform(records, 5070)
splicies <- split(records, records$taxon)[1:2]

splicies <- lapply(splicies, st_buffer, dist = 90)
st_buffer(x, dist = 90)
...
test <- st_intersects(splicies[[1]])

test1 <- test[ lengths(test) > 1 ]
untest <- unique(test1)

sum( lengths(test)  > 1 ) # 637 records
sum( lengths(test) == 2 ) # 285 records dupe in paired, 142 records...
sum( lengths(test)  > 3 ) 

test <- test[sum(lengths(test)  > 1 )]

out <- unique(test)

# identify each 

# find records < 90m distance 
