...
pos1 <- lapply(pos, function(x){as.numeric(sub(' ' , '', unlist(strsplit(x,","))))})
...
outwards <- lapply(pos, colon_balloon)




pts2remove1 <- pts2remove %>% 
  mutate(Values = str_extract(Species, 'c\\(([^\\)]+)\\)'),
         Species = str_remove(Species, ' c\\(([^\\)]+)\\)'),
         Species = str_squish(Species), 
         Values = str_remove(Values, 'c\\('), 
         Values = str_remove(Values, '\\)')) %>% 
  transform(Values = strsplit(Values, ",")) %>% 
  nest(bong = Values) %>% 
  unnest_longer(bong, indices_include = T)




fuck <- as.numeric(pts2remove$bong)
...
pts2remove1 <- select(pts2remove, Values) %>% 
  mutate(Values = str_remove(Values, 'c\\('), 
         Values = str_remove(Values, '\\)'))

avector <- pts2remove[,2]
...
spliris <- split(iris, f = iris$Species)
indices <- list(c(sample(1:50, size = 5)), c(sample(1:50, size = 7)), c(sample(1:50, size = 10)))

testv <- c('1', '5', '8', '9')

testv

spliris_l <- vector(mode = 'list', length = length(spliris))
for (i in seq_along(spliris)){
  spliris_l[[i]] <- spliris[[i]][-indices[[i]], ]
}


ott <- list(pts2remove1$Values)
ott1 <- unlist(ott)
# ott <- lapply(ott, as.numeric)
