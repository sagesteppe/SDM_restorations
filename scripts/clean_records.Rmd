...
filter(thinned, str_detect(taxon, '^K')) |> 
  pull(taxon) |> 
  unique() 
...
```{r}
pts1 <- filter(thinned, taxon == 'Koeleria macrantha')
...
OUTPUT <- filter(OUTPUT, !ID %in% c(1598, 1665, 404, 738, 112, 110, 1609, 184, 115))
# op <- st_intersection(OUTPUT, filter(st, STUSPS == 'MT'))

# 2920, 4757, 1155, 1151, 8028, 
