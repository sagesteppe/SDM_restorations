...
class(t)[1] == 'sf' 
unique(st_geometry_type(t))[1] == 'POINT'
...
if(class(x) == 'SpatRaster'){
  ...
  message('This class not expected by this small function. Small overhaul needed?')
  }

# this identified the occurrence records which we want to keep. 
which(
  !is.na(
    terra::extract(prov_stz[[2]], t, method = 'simple', ID = F)))

```

```{r generate projects for crews, eval= F}

unique(seed_zones$stz)
