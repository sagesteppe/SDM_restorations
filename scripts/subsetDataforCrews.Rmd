...
crs(example_rast) == crs(eSTZs)

p <- '/media/steppe/hdd/EmpiricalSeedZones/data/geodata/rasters'
f <- file.path(p, list.files(p, pattern = 'tif$'))
eSTZs <- rast(f[1])
plot(eSTZs)
plot(example_rast)
names(eSTZs) <- gsub('-.*tif$', '', basename(f[1]))

eSTZs1 <- resample(eSTZs, example_rast, method = 'near', threads = 16)
plot(eSTZs1)
rm(f, p)

lapply( 
  teams_list[13], project_maker, 
...
lapply(teams_list[c(7,13)], project_maker, 
       admu = admin_boundaries, 
       target_species = spp_needed, 
       blm_surf = blm_surf, 
       fs_surf = usfs_surf, 
...
lapply(seniors_list[3],  project_maker_original,
...
lapply(
  X = teams_list[c(7,13)], # 5 throws an error
  FUN = phen_tabulator,
  path = p2ts, 
  admu = admin_boundaries, 
  target_species = spp_needed
