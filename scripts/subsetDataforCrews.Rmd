...
occurrences <- bind_rows(
  ...
  mutate(
    ...
    date = str_extract(date, '[0-9]{4}')
    ) %>% 
  rename(year = date, coord_status = PtType) %>% 
  arrange(taxon, year, coord_status)

table(occurrences$coord_status)
st_write(occurrences, file.path(occ_p, 'jittered_occurrences.shp'), quiet = T)
...
allotments <- st_read('Public_land_ownership/BLM_Natl_Grazing_Allotment_Polygons/Allotment-simp.shp')

patches_p <- '/media/steppe/hdd/SDM_restorations/results/PatchesClippedBLM'
patches <- lapply(
  file.path(patches_p, list.files(patches_p, pattern = 'shp$')), st_read, quiet = T) |>
  bind_rows()
...
ggplot() + 
  geom_sf(data = BOWERSTZ, aes(fill = seed_zone)) +
  geom_sf(data = SWSTZ, aes(color = Zone))
