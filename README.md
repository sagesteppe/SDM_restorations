...
In order to identify patches which may represent metapopulations raster cells predicted as having less than 0.8 probability of suitable habitat were masked as NA's, and then areas which were crossed by streams with Strahler orders of three of more (DATASET), and the divides of HU10 watersheds (DATASET) were 'burnt' away from the raster. 
The resulting rasters were aggregated by a factor of 2 to 5, depending of their sizes (< 300 MiB 2, < 500 3, < 700 4, > 700 5) to accomodate, a rooks case search  using @LANDSCAPEMETRICS in a practical time period. 
